...
export default function Navbar() {
  ...
  // 1. Mark component as mounted (required for next-themes)
  useEffect(() => {
    setMounted(true)
  }, [])
        ...
        <div className="w-full px-6 lg:px-12">
          <div className="flex items-center justify-between h-18 lg:h-20">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}
                className="p-2.5 rounded-full hover:bg-accent/20 transition-colors text-foreground"
                aria-label="Toggle theme"
                disabled={!mounted}
              >
                <AnimatePresence mode="wait">
              ...
              <button
                onClick={() => setMobileOpen((v) => !v)}
                className="lg:hidden p-2.5 rounded-full hover:bg-accent/20 transition-colors text-foreground"
                aria-label="Toggle menu"
              >
                {mobileOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
              </button>
            ...
            <div className="flex flex-col gap-2">
                ...
                >
                  <Link
                    ...
                    className={`block py-4 text-2xl font-semibold ${
                      pathname === link.href ? 'text-emerald-500' : 'text-foreground'
                    }`}
                  >
                    {link.label}
                  </Link>
              ...
              >
                <Link
                  href="/get-started"
                  onClick={closeMobileMenu}
                  className="inline-flex items-center px-6 py-3 bg-emerald-500 text-white font-semibold rounded-full"
                >
                  Get Started
                </Link>
              </motion.div>
